...
class _AdminSignUpState extends State<AdminSignUp> {
  ...
  bool isSigningUp = false; // Track signup state
  Future<void> _signUp() async {
    try {
      ...
      if (user != null) {
        ...
        log('Email uploaded to Firestore.');
      } else {
        log('User is null.');
      }

      // If signup is successful, show a success notification
      ...
      // Upload the user's email to Firestore under the "Admin" collection
      if (user != null) {
        await FirebaseFirestore.instance.collection('Admin').doc(user.uid).set({
          'email': user.email,
        });
      ...
      log(error.toString());
      // Handle signup errors here, e.g., email already exists
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: isSigningUp
                          ? null
                          : _signUp, // Disable button during signup
